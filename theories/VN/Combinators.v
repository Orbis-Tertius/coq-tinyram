...
Module Combinators (Params : Types.TinyRAMParameters).
  ...
  Fixpoint afterBranch {A: Type} (is : list Instruction) (bch : branch A) : block A :=
    match is with
    | nil => bbb bch
    | instr :: instrs => bbi instr (afterBranch instrs bch)
    end.

  (* SAZ: rationalize the names of the combinators? *)
  (** Another combinator that appends a list of instructions to the beginning of a
      block *)
  Fixpoint blk_append {lbl : Type} (l : list Instruction) (b:block lbl) : block lbl :=
    match l with
    | [] => b
  ...
  Section Correctness.
    ...
    Definition denote_list: list Instruction -> itree E unit :=
     ...
     *)
    Lemma denote_afterBranch :
      forall {label} instrs (b: branch (fin label)),
        denote_bk (afterBranch instrs b) ≅ (denote_list instrs ;; denote_br b).
    Proof.
    ...
    Lemma denote_blk_append : forall (lbl : nat) (l : list Instruction) (b:block (fin lbl)),
    ...
    Context `{HasFFlag' : FFlag -< E}.
    Context `{HasPC' : PC -< E}.
    Context `{BimapBks : Bimap nat bks Nat.add}.
    Context `{Eq2Bks : Eq2 nat bks}.
    Hint Mode Eq2 + ! : typeclass_instances.
    Hint Mode Bimap + ! ! : typeclass_instances.

    (*Lemma app_bks_correct : forall {A B C D : nat} (ab: bks A B) (cd: bks C D),
        let denote_bks' := @denote_bks E' HasRegs' HasExit' HasFFlag' HasPC' in
        denote_bks' (A + C) (B + D) (app_bks ab cd) ⩯ bimap (denote_bks' A B ab) (denote_bks' C D cd).
    Proof.
